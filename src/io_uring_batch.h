 ...
 * iteration.
 *
 * PROOF-OF-CONCEPT. Readiness detection stays with epoll; only the data
 * movement is batched, so every recv/send is issued MSG_DONTWAIT against an
 ...
 * stops batching once the EWMA passes IO_URING_ADAPTIVE_HIGH_CMDS,
 * resuming below IO_URING_ADAPTIVE_LOW_CMDS (hysteresis).
 *
