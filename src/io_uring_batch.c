 ...
 * DESIGN NOTE
 *
 * Valkey's main-thread loop costs, for N ready clients per iteration:
...
/* A client whose recv is already queued in the open batch must not take the
 * synchronous path too: the SQE holds the query buffer's address, and a
 * synchronous read could move it (invariant 6). The pending recv covers
 * this readiness, so report the read as handled. This is reached when a
 * nested event loop (processEventsWhileBlocked) polls the same fd again
 * before the outer pass flushes. */
static inline int alreadyQueued(client *c) {
    return ring_ok && c->io_uring_slot != -1;
}
 ...
 * (query buffer, c->buf) that freeClient() is about to free, and its fd,
 * which freeClient() closes and a later accept in the same pass may reuse.
 * Rewrite it in place into a NOP so the kernel never touches either. The
 * user_data is kept so submitAndReap() still accounts for its CQE. After
 * submit (in_flush) the CQE has already been applied and clearing the
 * slot's client is enough. */
...
void ioUringBatchClientFreed(client *c) {
    ...
    if (idx < nreads && rslots[idx].c == c) {
        ...
        cancelSlot(s, (uintptr_t)idx);
        return;
    }
    if (idx < nwrites && wslots[idx].c == c) {
        cancelSlot(&wslots[idx], (uintptr_t)idx);
        return;
    }
}
