 ...
 *     (the kernel, not the client, would pick the destination buffer). This
 *     module keeps the existing buffer ownership rules unchanged.
 ...
 *      a module's RM_Call from an event-loop callback...). freeClient()
 *      calls ioUringBatchClientFreed(B), which detaches B's slot so its
 *      deferred handler is skipped and, if B was reading into a pooled
 *      buffer, hands the buffer back before freeClient's sdsfree() would
 *      double-free it. If B's SQE has not been submitted yet it is rewritten
 *      in place into a NOP: it carries the addresses of B's query buffer or
 *      reply buffers, which freeClient() frees, and B's fd, which
 *      freeClient() closes and an accept later in the same pass may reuse,
 *      so submitting it would recv into freed memory or send freed memory
 *      to a different connection. (Main thread only: a client inside an
 *      I/O-thread batch has io_*_state == PENDING_IO and freeClient()
 *      defers it with freeClientAsync().)
 ...
 *   6. A nested event loop can run between a main-thread queue and its
 *      flush (a synchronous-path handler in the same fd pass starts one:
 *      rdbLoad on a replica's full sync, DEBUG RELOAD, a busy script).
 *      epoll is level-triggered, so the queued client's fd fires again in
 *      the nested poll; readQueryFromClient() must then treat the client as
 *      handled (ioUringBatchQueueRead returns 1 for a client already in the
 *      batch) instead of taking the synchronous path, which could realloc
 *      the query buffer the unsubmitted SQE points into. The nested loop's
 *      own after-events hook flushes the outer batch; the tails then run
 *      under ProcessingEventsWhileBlocked exactly as the same clients' reads
 *      would have in the unmodified nested loop.
 *   7. Inside an I/O thread the job is counted finished (io_jobs_finished)
...
typedef struct iouSlot {
    ...
    struct io_uring_sqe *sqe; /* the queued SQE, valid until submit (see invariant 2) */
...
/* A client whose recv is already queued in this thread's open batch must
 * not take the synchronous path too: the SQE holds the query buffer's
 * address, and a synchronous read could move it (invariant 6). The pending
 * recv covers this readiness, so report the read as handled. This is
 * reached when a nested event loop (processEventsWhileBlocked) polls the
 * same fd again before the outer pass flushes. */
static inline int alreadyQueued(client *c) {
    return T && c->io_uring_slot != -1;
}

int ioUringBatchQueueRead(client *c) {
    if (alreadyQueued(c)) return 1;
...
/* Invariant 2: an SQE that has not been submitted yet carries raw addresses
 * (query buffer, c->buf, reply blocks) that freeClient() is about to free,
 * and its fd, which freeClient() closes and a later accept in the same
 * pass may reuse. Rewrite it in place into a NOP so the kernel never
 * touches either. The user_data is kept so submitAndReap() still accounts
 * for its CQE. After submit (in_flush) the CQE has already been applied and
 * clearing the slot's client is enough. */
static void cancelSlot(iouSlot *s, uintptr_t ud) {
    if (!T->in_flush && s->sqe) {
        io_uring_prep_nop(s->sqe);
        io_uring_sqe_set_data(s->sqe, (void *)ud);
    }
    s->sqe = NULL;
    s->c = NULL;
}
...
void ioUringBatchClientFreed(client *c) {
    ...
    if (idx < T->nreads && T->rslots[idx].c == c) {
        ...
        cancelSlot(s, (uintptr_t)UD_READ(idx));
        return;
    }
    if (idx < T->nwrites && T->wslots[idx].c == c) {
        cancelSlot(&T->wslots[idx], (uintptr_t)UD_WRITE_OF(idx)); /* wc is returned by the flush loop */
        return;
    }
}
