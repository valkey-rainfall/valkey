...
int canCommandBeOffloaded(int slot, struct serverCommand *cmd) {
    ...
    /* Modules may register hooks (keyspace notifications, filters, data types) that must run
     * on the main thread. The built-in static modules (e.g. the Lua engine) register none. */
    if (moduleCountDynamic() > 0 && !server.io_threads_do_commands_offloading_with_modules) return 0;
    if (!CMD_CAN_BE_OFFLOADED(cmd)) return 0;
