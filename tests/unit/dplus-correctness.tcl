...
# Always-on engagement counter: GETs answered on the owning IO thread without
# main. Unlike dplus_speculative_hits it does not need the instrumented build.
proc tier_reads {r} {
    set payload [$r info fastpath]
    if {![regexp {fastpath_speculative_reads:(\d+)} $payload -> value]} {
        fail "missing fastpath_speculative_reads in INFO fastpath"
    }
    return $value
}

# A deferring client that stays on the fast path: the harness's
# valkey_deferring_client runs SELECT 9 first, and on this transport one
# non-fast-path command detaches the connection for life (only AUTH readmits).
# Such a client can never reach the tier, so the tier tests open a raw client
# that sends GET as its first command and reads db 0; the control client is
# pinned to db 0 to match.
proc fp_deferring_client {} {
    return [valkey [srv 0 host] [srv 0 port] 1 $::tls]
}

# Pump pipelined GETs until the speculative tier demonstrably answers for this
# deferring client (its speculative prefix must be open: no entry on main).
proc ensure_engaged {r rd key} {
    set before [tier_reads $r]
    for {set round 0} {$round < 20} {incr round} {
        for {set i 0} {$i < 10} {incr i} { $rd get $key }
        for {set i 0} {$i < 10} {incr i} { $rd read }
        if {[tier_reads $r] > $before} return
    }
    fail "speculative path never engaged for test client"
}

start_server {tags {"dplus-correctness"} overrides {io-threads 4 io-threads-speculative-reads yes save {}}} {
    r select 0
