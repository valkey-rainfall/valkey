...
# io_uring support (Linux only). Mirrors USE_IO_URING=yes in src/Makefile:
# links liburing and defines HAVE_LIBURING; anything else builds the no-op stubs.
if (USE_IO_URING)
    if (LINUX AND NOT APPLE)
        valkey_parse_build_option(${USE_IO_URING} IO_URING_ENABLED)
        if (IO_URING_ENABLED EQUAL 1)
            valkey_pkg_config(liburing URING_LIBS)
            add_valkey_server_compiler_options("-DHAVE_LIBURING")
            list(APPEND SERVER_LIBS "${URING_LIBS}")
            message(STATUS "io_uring support is enabled")
        else ()
            message(STATUS "io_uring support is disabled")
        endif ()
    else ()
        message(WARNING "io_uring is only supported on Linux platforms")
    endif ()
else ()
    message(STATUS "io_uring support is disabled")
endif ()
...
unset(USE_IO_URING CACHE)
unset(IO_URING_ENABLED CACHE)
